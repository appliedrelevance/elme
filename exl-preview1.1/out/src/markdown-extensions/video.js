...
function video_plugin(md, name, options) {
    options = options || {};
    let min_markers = 1, marker_str = options.marker, marker_len = marker_str.length, marker_char = marker_str.charCodeAt(0), validate = options.validate, render = options.render;
    function container(state, startLine, endLine, silent) {
        let pos, nextLine, marker_count, markup, params, token, old_parent, old_line_max, auto_closed = false, start = state.bMarks[startLine] + state.tShift[startLine], max = state.eMarks[startLine];
