            ...
            chunk.children
              ...
              .map((text) => {
                ...
                if (content == null || content.length == 0) {
                  content = fullLooseMatches.filter(
                    (regexMatch) => regexMatch == text.line
                  );
                }
                if (content == null || content.length == 0) {
                  return;
                ...
                if (rangeMatch && idMatch) {
                  ...
                  const allowedValues = rangeMatch[1].match(
                    common.allowedRangeValues
                  );
                  if (!allowedValues) {
                    onError({
                      lineNumber: text.lineNumber,
                ...
                attrMatches.forEach((attr) => {
                  if (attr) {
                    ...
                    if (common.allowedCodeAttributes.indexOf(attr) === -1) {
                      onError({
                        lineNumber: text.lineNumber,
                        detail: detailStrings.codeNonAllowedAttribute.replace(
                          "___",
                          attr
                        ),
                        context: text.line,
                ...
                const interactiveMatches = content.match(
                  common.codeInteractiveMatch
                );
