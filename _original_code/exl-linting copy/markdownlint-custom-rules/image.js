          ...
          .forEach(function forChild(text) {
            ...
            imageMatches.forEach((imageMatch) => {
              const content = !imageMatch.match(common.imageComplexEndTagMatch)
                ? fullLooseMatches.filter((match) =>
                    match.includes(imageMatch)
                  )[0]
                : imageMatch;
              ...
              //source check
              const notImageComplexEndTagMatch = content.match(
                common.imageComplexEndTagMatch
              );
              ...
              attrMatches.forEach((attr) => {
                ...
                if (
                  ...
                  onError({
                    lineNumber: text.lineNumber,
                    detail: detailStrings.imageNonAllowedAttribute.replace(
                      "___",
                      attr
                    ),
                    context: text.line,
              ...
              if (typeMatch) {
                if (allowedImageTypes.indexOf(typeMatch[1]) === -1) {
                  onError({
                    lineNumber: text.lineNumber,
                    detail: detailStrings.imageNonAllowedType.replace(
                      "___",
                      typeMatch[1]
                    ),
                    context: text.line,
                ...
                if (typeMatch[1] === "complex") {
                  ...
                  if (
                    ...
                    !content.match(common.imageComplexEndTagMatch)
                  ) {
                    onError({
                      lineNumber: text.lineNumber,
                  ...
                  const longDescMatch = common.imageLongDescriptionMatch.exec(
                    content
                  );
                  if (
                    !longDescMatch ||
                    longDescMatch[4].match(/^\s*$/im)[0] != ""
                  ) {
                    onError({
                      lineNumber: text.lineNumber,
