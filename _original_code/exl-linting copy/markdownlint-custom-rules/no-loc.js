        ...
        inline.children
          ...
          .forEach(function forChild(text) {
            ...
            noLocMatches.forEach((content) => {
              // no dash for "no-loc"
              if (
                content.match(common.openNoDashNoLoc) &&
                !content.match(common.syntaxNoLoc)
              ) {
                onError({
              ...
              if (
                content.match(common.openNoLoc) &&
                !content.match(common.syntaxNoLoc)
              ) {
                //different errors
                //missing text attribute
                let attrMatches = content.match(
                  common.missingTextAttributeNoLoc
                );
                ...
                attrMatches.forEach((attr) => {
                  ...
                  if (common.allowedNoLocAttributes.indexOf(attr) === -1) {
                    onError({
                      lineNumber: text.lineNumber,
                      detail: detailStrings.NoLocNonAllowedAttribute.replace(
                        "___",
                        attr
                      ),
                      context: text.line,
