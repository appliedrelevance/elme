...
function isMarkdownFileCheck(editor, languageId) {
    if (editor.document.languageId !== "markdown") {
        if (editor.document.languageId !== "yaml") {
            postInformation("The exl-markdown extension only works on Markdown files.");
        }
        return false;
    }
    else {
...
function extractDocumentLink(document, link, matchIndex) {
    ...
    try {
        ...
        if (httpMatch) {
            ...
            const documentLink = new vscode.DocumentLink(new vscode.Range(linkStart, linkEnd), vscode.Uri.file(path.resolve(filePath, link)));
            return documentLink;
        }
    }
    catch (e) {
